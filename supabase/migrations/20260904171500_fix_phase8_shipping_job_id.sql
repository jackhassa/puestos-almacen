...
-- ============================================================================
-- Puestos Almacen - FASE 8.1
-- Correccion del RPC de estadisticas:
-- shipping_metrics expone shipping_job_id, no id.
--
-- NO modifica la migracion FASE 8 ya aplicada.
-- ============================================================================
...
begin
  ...
  cart_cycles as (
    select
      shipping.shipping_job_id as shipping_job_id,
      shipping.operational_date,
...
begin
  if to_regprocedure('public.warehouse_statistics_get_snapshot(date,date)') is null then
    raise exception 'No existe warehouse_statistics_get_snapshot tras FASE 8.1.';
  end if;
end;
$$;
